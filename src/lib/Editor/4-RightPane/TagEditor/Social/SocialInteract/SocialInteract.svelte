<script>
	...
	import TextBlock from '$lib/Editor/4-RightPane/TagEditor/Shared/TextBlock.svelte';
	import NumberBlock from '$lib/Editor/4-RightPane/TagEditor/Shared/NumberBlock.svelte';
	export let data;
...
<div class="container">
	<div class="blocks-container">
		{#if data && data?.length > 0 && index < data?.length}
			<TextBlock title="Interaction Link" {index} bind:data={data[index]['@_uri']} />

			<div class="block">
				<label>
					<div class="heading">
						<h4 style={`color: hsla(${352 - ((index + 1) % 6) * 60}, 100%, 33%, 1)`}>Platform</h4>
					</div>
			...
			<TextBlock title="Platform Account ID" {index} bind:data={data[index]['@_accountId']} />
	...
	<IndexScroll bind:data bind:index blank={blankSocialInteract} />
</div>
