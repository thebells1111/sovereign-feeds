...
{#if $rssData}
	<div class="funding header">
		<h3>{$tagHeaders.podcast.funding.title}</h3>
		<button
			on:click={() => {
		...
		>
			Add New {`${$tagHeaders.podcast.funding.block}`}
		</button>
	</div>
	{#each fundingArray as funding, i}
		<div class="funding block">
			<h4 style={`color: hsla(${352 - ((i + 1) % 4) * 90}, 100%, 33%, 1)`}>
				{`${$tagHeaders.podcast.funding.block} `} #{i + 1}:
			</h4>
			<button
				on:click={(e) => {
		...
		<label>
			<h5 style={`color: hsla(${352 - ((i + 1) % 4) * 90}, 100%, 33%, 1)`}>
				{$tagHeaders.podcast.funding.text}
			</h5>
			<input type="text" bind:value={funding['#text']} />
		</label>
		<label>
			<h5 style={`color: hsla(${352 - ((i + 1) % 4) * 90}, 100%, 33%, 1)`}>
				{$tagHeaders.podcast.funding.url}
			</h5>
