<script>
	...
	export let blur = async () => {
		...
		let podcast = (await editorDB.getItem(`${$selectedPodcast.id}`)) || {};
		podcast.maxEpisodes = value;
		$maxEpisodes = value;
		$filteredEpisodesList = $episodesList.slice(0, value);
		editorDB.setItem(`${$selectedPodcast.id}`, podcast);
	};
	export let focus = () => {
		value = '';
	};
