<script>
  ...
  import { podcastDB, podcastStateDB, seenEpisodesCounts } from '$/stores';
  ...
  function removeDatum(index) {
    let removedPodcast = data.splice(index, 1)[0];

    data = data;
    podcastDB.removeItem(`${removedPodcast.id}`);
    delete $seenEpisodesCounts[removedPodcast.id];
    podcastStateDB.removeItem(`${removedPodcast.id}`);
  }
</script>
